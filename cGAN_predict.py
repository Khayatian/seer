...
def get_generator(weather_layer, condition_layer, noise_layer):
    ...
    # cond = Reshape((24, 6, 5))(RepeatVector(24 * 6)(condition_layer))
    # # Prep weather condition
    # hid = Reshape((6, 24, 4))(RepeatVector(6)(weather_layer))
    # hid = (Permute((2, 1, 3), input_shape=(6, 24, 4)))(hid)
    # hid = Concatenate()([hid, cond])  # [9,24,6]
    # # Convolution
    # hid = Conv2D(4, kernel_size=(48, 12), strides=(2, 2), padding='same',
    #              activity_regularizer=regularizers.l1(10e-5))(hid)  # to [12,3]
    # hid = ELU(alpha=1)(hid)
    # hid = Conv2D(2, kernel_size=(24, 6), strides=(2, 2), padding='same',
    #              activity_regularizer=regularizers.l1(10e-5))(hid)  # to [6,2]
    # hid = ELU(alpha=1)(hid)
    # hid = Flatten()(hid)
    # #  infusing latent noise
    # hid = Concatenate()([hid, noise_layer])
    ...
    # Convolution
    hid = Conv2D(8, kernel_size=(48, 12), strides=(1, 1), padding='same')(hid)  # to [6,6]
    hid = ELU(alpha=1)(hid)
    hid = Conv2D(4, kernel_size=(48, 12), strides=(1, 1), padding='same')(hid)  # to [12,6]
    hid = ELU(alpha=1)(hid)
    hid = Conv2D(2, kernel_size=(48, 12), strides=(1, 1), padding='same')(hid)  # to [24,6]
    hid = ELU(alpha=1)(hid)
...
def get_discriminator(perf_layer, condition_layer, weather_layer):
    # Convolution
    hid1 = Conv2D(16, (48, 12), strides=(1, 1), padding='same')(perf_layer)
    hid1 = ELU(alpha=1)(hid1)
    hid2 = Conv2D(8, (12, 6), strides=(1, 1), padding='same')(perf_layer)
    hid2 = ELU(alpha=1)(hid2)
    hid3 = Conv2D(8, (6, 3), strides=(1, 1), padding='same')(perf_layer)
    hid3 = ELU(alpha=1)(hid3)
    hid4 = Conv2D(8, (3, 3), strides=(1, 1), padding='same')(perf_layer)
...
def generate_images(batch_id, noise_fuse):
    prfm = x_performance[batch_id * BATCH_SIZE: (batch_id + 1) * BATCH_SIZE] * \
           np.random.uniform(0.999 - noise_fuse, 1.001 + noise_fuse, size=(1, 1))
    return prfm
...
# Load inputs
mat = scipy.io.loadmat('build1.mat')  # All inputs are stored in a Matlab data file named "build1.mat"
...
noise_input = Input(shape=(96,))  # input_x: shape of latent noise (96)
...
for epoch in range(N_EPOCHS):
    ...
    for batch_idx in range(num_batches):
        ...
        noise_flip = 0.05  # percentage of labels to flip
        ...
        noise_data = generate_noise(BATCH_SIZE, 96)
        ...
        noise_data = generate_noise(BATCH_SIZE, 96)
        labels = y_operation[batch_idx * BATCH_SIZE: (batch_idx + 1) * BATCH_SIZE]
        g_loss = gan.train_on_batch([weather_data, random_labels, noise_data, labels, weather_data],
...
save_path = 'C:/Users/khfa/PycharmProjects/GANS/'
name_of_file = 'cGAN_perf_CiLr_build1a'
save_path = os.path.join(save_path, name_of_file+".mat")
scipy.io.savemat(save_path, {'cGAN_perf': perform_list})
generator.save('CGAN_CiLr_build1a.h5')
